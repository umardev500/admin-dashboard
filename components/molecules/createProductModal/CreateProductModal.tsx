...
export const CreateProductModal: React.FC<Props> = ({ name, modalSetState, callback }) => {
    ...
    // check status updating or posting
    const isUpdating = name !== '' && name !== undefined

    useEffect(() => {
        if (isUpdating) {
            if (catRef.current != null) catRef.current.value = name
        }
    }, [])
